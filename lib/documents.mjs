...
export function generateBusiness(demand, planning) {
  ...
  const markdown = `# ${title} 사업설명자료\n\n> 본 초안은 업로드된 기술수요조사서와 기술기획보고서만 기술 사실의 근거로 사용했습니다. 기존 사업설명자료는 목차·공공문서 문체·표 구성에만 참고했습니다.\n\n## 1. 추진배경\n\n${contentOr(background, '기술기획보고서의 추진배경 근거 보완')}\n\n${sourceTag(planning, '추진배경')}\n\n## 2. 사업개요\n\n| 구분 | 내용 | 근거 |\n| --- | --- | --- |\n| 사업명 | ${escapeMd(title)} | ${escapeMd(demand.filename)} · 수요명 |\n| 사업목표 | ${escapeMd(contentOr(goal, '목표 확정 필요'))} | ${escapeMd(demand.filename)} · 개발목표 |\n| 사업기간 | ${escapeMd(contentOr(period, '기간 확정 필요'))} | ${escapeMd(demand.filename)} · 기간 |\n| 사업비 | ${escapeMd(contentOr(budget, '예산 확정 필요'))} | ${escapeMd(demand.filename)} · 금액 |\n| 추진방식·수행체계 | 담당자 결정 필요 | 업로드 문서 확인 필요 |\n\n## 3. 사업 필요성 및 정부지원 필요성\n\n${contentOr(need, '필요성·정부지원 논리의 공식 근거 보완')}\n\n${sourceTag(demand, '지원 필요성/기대효과')}\n\n## 4. 주요 연구개발 내용\n\n${contentOr(contents, '주요 연구개발 내용 확정 필요')}\n\n${sourceTag(demand, '개발내용')}\n\n## 5. 사업 범위와 추진전략\n\n${contentOr(scope, '포함·제외 범위 확정 필요')}\n\n${contentOr(strategy, '단계·역할·검증 절차 확정 필요')}\n\n${sourceTag(planning, '개념·범위/추진전략')}\n\n## 6. 정책·기술·산업 연계\n\n${contentOr(section(planning.markdown, [/사업\s*근거/, /정책.*동향/, /시장.*기술/]), '공식 정책·시장·기술 근거 조사 필요')}\n\n${sourceTag(planning, '사업근거/주요동향')}\n\n## 7. 예산 및 성과관리\n\nㅇ 기간·예산은 위 사업개요의 원문 값을 기준으로 하며, 연차별 배분은 근거가 없으면 확정하지 않음.\n\nㅇ 대표 성과지표는 기술성과·통합성·성능·안전성·상호운용성을 중심으로 구성하되 목표값·산식·시험환경·검증주체를 함께 확정해야 함.\n\n${sourceTag(planning, '투자계획/성과관리')}\n\n## 8. 기대효과\n\n${contentOr(effects || need, '기술·산업·공공 파급경로 보완 필요')}\n\n${sourceTag(effects ? planning : demand, '기대효과')}\n\n## 9. 검토 필요사항\n\n${unresolved.map(x => `- [담당자 결정 필요] ${x}`).join('\n') || '- 업로드 문서에서 주요 결정값을 확인했으며 최종 검토가 필요함'}\n\n## 출처 및 작성 메모\n\n- 기술 사실 1차 근거: ${demand.filename} (SHA-256: ${demand.sha256})\n- 기획 논리 근거: ${planning.filename} (SHA-256: ${planning.sha256})\n- 형식 참고 원칙: 사업설명자료 HWP의 구조·문체·표만 참고하며 기술명·기관명·수치·정책문구는 가져오지 않음\n`;
  return { markdown, title: `${title}_사업설명자료`, unresolved, provenance: [demand, planning].map(({role, filename, sha256}) => ({role, filename, sha256})) };
}

export function generateTask(rfp) {
