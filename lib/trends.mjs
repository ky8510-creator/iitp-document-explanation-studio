...
export function generateTrendMaterial(workflow, analysis) {
  ...
  const markdown = `# ${md(title)}\n\n> 본 초안은 기술동향 분석에서 분리 보존한 실시간 RSS 근거와 업로드 HWPX 근거만 사용했습니다. RSS 요약은 원문 발견 근거이며 확정 사실이 아닙니다. 출처 없는 정책, 예산, 기간, KPI, TRL, 기관, 실증·배포 대상 및 정량값은 만들지 않았습니다.\n\n## 1. ${isBusiness ? '추진배경과 기획 필요성' : '과제 배경과 해결 문제'}\n\n- ${md(analysis.fieldLabel)} 분야의 \`${md(analysis.topic)}\` 관련 공개 근거를 수집했으며, 아래 T 번호 출처의 제목·RSS 설명 범위에서만 검토함.\n- [추가 조사 필요] 기술·산업·정책상 필요성은 연결된 원문, 공식 통계, 수요조사로 확인해야 함.\n\n## 2. 기술동향 근거\n\n${evidenceDigest(analysis)}\n\n## 3. 업로드 문서 근거\n\n${analysis.sourceDocument ? `- ${md(analysis.sourceDocument.filename)} · SHA-256 ${analysis.sourceDocument.sha256}\n- 문서에서 추출한 세부 내용은 부록의 원 분석 D1을 참조하고 공개 웹 근거와 별도로 검증해야 함.` : '- 선택 HWPX가 없어 내부 문서 근거는 포함하지 않음.'}\n\n## 4. ${isBusiness ? '사업 목표·범위 초안' : '과제 목표·개발내용 초안'}\n\n- [담당자 확인 필요] 해결할 문제, 수요자, 최종 목표와 포함·제외 범위를 확정해야 함.\n- [추가 조사 필요] 후보 기술의 성능·안전·보안·상호운용 요구사항을 공식 원문과 시험 근거로 구체화해야 함.\n- [담당자 확인 필요] 동향 근거를 실제 지원 필요성 또는 연구개발 요구사항으로 전환할지 검토해야 함.\n\n## 5. 실행조건 및 확인 필요값\n\n| 항목 | 상태 |\n| --- | --- |\n${confirmationRows}\n\n## 6. ${isBusiness ? '성과·파급효과 검토' : '성과판정·활용 검토'}\n\n- [추가 조사 필요] 기대효과는 기준선·인과경로·수혜대상 근거를 확보한 뒤 작성해야 함.\n- [담당자 확인 필요] 성과지표와 완료판정 조건은 측정 가능성과 독립 검증 방법을 포함해 확정해야 함.\n\n## 7. 미해결 사항\n\n${unresolved.map(item => `- [확인/조사 필요] ${md(item)}`).join('\n')}\n\n## 부록. 기술동향 분석 원문\n\n${analysis.markdown}\n`;
  ...
  return { markdown, title: title.replace(/[\\/:*?"<>|]/g, '_'), unresolved, provenance };
}

export const generateBusinessTrend = analysis => generateTrendMaterial('business', analysis);
export const generateTaskTrend = analysis => generateTrendMaterial('task', analysis);
