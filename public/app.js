...
function choose(role,file) {
  if (!file) return;
  if (!/\.hwpx$/i.test(file.name)) return toast('HWPX 파일만 선택할 수 있습니다.','error');
  if (file.size>30*1024*1024) return toast('파일은 30MB 이하여야 합니다.','error');
...
async function download(){const button=$('#downloadBtn');button.classList.add('busy');button.setAttribute('aria-busy','true');button.textContent='Kordoc 검증·변환 중…';try{const response=await request('/api/export',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({markdown:$('#editor').value,filename:state.result.title})});const blob=await response.blob();const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`${state.result.title}.hwpx`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);setStep(3);toast(`구조 검증 완료 · 재추출 ${response.headers.get('x-kordoc-roundtrip-chars')||'-'}자`)}catch(error){toast(error.message,'error')}finally{button.classList.remove('busy');button.removeAttribute('aria-busy');button.innerHTML='검증 후 HWPX 다운로드 <span>↓</span>'}}

document.querySelectorAll('.tab').forEach(tab=>tab.onclick=()=>{document.querySelectorAll('.tab').forEach(x=>{const active=x===tab;x.classList.toggle('active',active);x.setAttribute('aria-pressed',String(active));x.querySelector('.tab-state').lastChild.textContent=active?' 선택됨':' 선택'});state.workflow=tab.dataset.workflow;reset()});
